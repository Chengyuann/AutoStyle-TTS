...
from reformat_data_ft_llm_cn import process
...
def main(args):
    ...
    if args.save_details:
        with open(args.details_output_path, "w", encoding='utf-8') as f:
            json.dump({"f1_weighted": f1, "detail_pred": details}, f, ensure_ascii=False, indent=2)
        print(f"Detailed predictions saved to {args.details_output_path}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate Base Llama Model F1 Score on Chinese Dataset')
    parser.add_argument('--base_model_id', type=str, default='/apdcephfs_cq10/share_1615176/cq2/macy/BiosERC/llm_model/Qwen2.5-7B-Instruct', help='Base LLM model path')
    parser.add_argument('--data_name', type=str, help='Data name in {iemocap, meld, emorynlp}', default='iemocap')
    parser.add_argument('--data_folder', type=str, help='Path to the data folder', default='/apdcephfs_cq10/share_1615176/cq2/macy/BiosERC/cndata')
    parser.add_argument('--output_folder', type=str, help='Path to save outputs', default='./finetuned_llm_cn/')
    ...
    parser.add_argument('--details_output_path', type=str, default='base_model_evaluation_details_cn.json', help='Path to save detailed predictions')
